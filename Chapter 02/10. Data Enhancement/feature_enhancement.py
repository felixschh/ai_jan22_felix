...
#adding new feature t_dif
data['t_dif'] = data.apply(lambda row: abs(row['t1']- row['t2']), axis=1)
...
def data_enhancement(data):
    ...
    for season in data['season'].unique():
        ...
        t_dif_std = seasonal_data['t_dif'].std()
        ...
        for i in gen_data[gen_data['season'] == season].index:
            ...
            if np.random.randint(2) == 1:
                gen_data['t2'].values[i] += t_dif_std/10
            else:
                gen_data['t2'].values[i] -= t_dif_std/10
...
num_vars = ['t1','t2','hum','wind_speed', 't_dif']
